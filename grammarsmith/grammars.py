...
def build(patterns, guards=None, timeout=90):
    """Generate + compile a mini-grammar. Content-addressed by (patterns, guards). Returns (so, symbol)."""
    if not patterns:
    ...
    with open(os.path.join(d, 'grammar.js'), 'w') as f:
        f.write(_grammar_src(name, patterns, guards))
    env = _env()
    g = subprocess.run(['timeout', str(timeout), 'tree-sitter', 'generate'],
                       cwd=d, env=env, capture_output=True, text=True)
    if g.returncode != 0:
        raise RuntimeError(f"generate failed for {patterns}: {(g.stderr or g.stdout)[-400:]}")
    cc = subprocess.run(['timeout', str(timeout), 'cc', '-O2', '-fPIC', '-shared',
                         '-I', 'src', 'src/parser.c', '-o', so],
                        cwd=d, env=env, capture_output=True, text=True)
    if cc.returncode != 0:
        raise RuntimeError(f"compile failed for {patterns}: {(cc.stderr or cc.stdout)[-400:]}")
    return so, symbol
